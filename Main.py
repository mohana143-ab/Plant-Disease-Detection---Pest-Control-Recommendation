...
labels = ['Chilli__Bacterial_spot', 'Chilli_healthy','Cotton_Black_rot', 'Cotton_Esca(Black_Measles)', 'Cotton__healthy','Cotton_Leaf_blight(Isariopsis_Leaf_Spot)', 

         'Rice__Brownspot', 'Rice_Healthy', 'Rice_Leafblast', 'Rice_Leafblight',  'Tomato_Bacterial_spot', 'Tomato__Early_blight',
          'Tomato__healthy', 'Tomato_Late_blight', 'Tomato_Leaf_Mold', 'Tomato__Septoria_leaf_spot',
          'Tomato__Spider_mites Two-spotted_spider_mite', 'Tomato_Target_Spot', 'Tomato_Tomato_mosaic_virus', 'Tomato__Tomato_Yellow_Leaf_Curl_Virus']
...
main = tkinter.Tk()
main.title("Plant Disease Detection using Machine Learning and Deep Learning  Algorithms") #designing main screen
...
def predict():
    ...
    cv2.putText(img, 'Plant Disease Predicted as : '+labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (255, 0, 0), 2)
    cv2.putText(img, 'Recommended Fertilizer is : '+fertilizers[predict], (10, 40),  cv2.FONT_HERSHEY_SIMPLEX,0.5, (255, 0, 0), 1)
    cv2.imshow('Plant Disease Predicted as : '+labels[predict], img)
    cv2.waitKey(0)

font = ('times', 16, 'bold')
title = Label(main, text='Plant Disease Detection using Machine Learning and Deep Learning  Algorithms')
...
svmButton = Button(main, text="Run SVM Algorithms", command=runSVM)
svmButton.place(x=750,y=500)
svmButton.config(font=font1)

fcnnButton = Button(main, text="Run DNN Algorithms", command=runDNN)
fcnnButton.place(x=50,y=550)
fcnnButton.config(font=font1) 

cnnButton = Button(main, text="Run Propose CNN Algorithm", command=runCNN)
cnnButton.place(x=370,y=550)
...
predictButton = Button(main, text="Disease Detection and Recommend Fertilizer from Test Image", command=predict)
